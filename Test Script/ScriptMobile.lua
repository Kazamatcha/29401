pcall(function()
    ...
    local oldGui = playerGui and playerGui:FindFirstChild("DraconicHubGui")
    if oldGui then
        oldGui:Destroy()
    end
end)
...
local DRACONIC_THEME_URL = "https://raw.githubusercontent.com/Ksjsowos/29401/refs/heads/main/Test%20Script/DraconicTheme.lua" -- Paste your hosted theme URL here.

local okGet, sourceOrErr = pcall(function()
    return game:HttpGet(DRACONIC_THEME_URL)
end)
if not okGet or type(sourceOrErr) ~= "string" or sourceOrErr == "" then
    warn("Failed to download theme: " .. tostring(sourceOrErr))
    return
end

local themeChunk, loadErr = loadstring(sourceOrErr)
if not themeChunk then
    warn("Failed to compile theme: " .. tostring(loadErr))
    return
end

local okRun, loadedTheme = pcall(themeChunk)
if not okRun or type(loadedTheme) ~= "table" or type(loadedTheme.BuildTheme) ~= "function" then
    warn("Invalid theme module: " .. tostring(loadedTheme))
    return
end

local DRACONIC_THEME_NAME = loadedTheme.ThemeName or "DraconicButtons"

local function forceApplyDraconicTheme(accentColor)
    local accent = accentColor or Color3.fromRGB(255, 0, 0)
    local themeTable = loadedTheme.BuildTheme(accent)
    Fluent.Themes[DRACONIC_THEME_NAME] = themeTable

    if type(Fluent.LoadedThemes) == "table" then
        Fluent.LoadedThemes[DRACONIC_THEME_NAME] = themeTable
    end

    local okNamed = pcall(function()
        Fluent:SetTheme(DRACONIC_THEME_NAME)
    end)

    if not okNamed then
        pcall(function()
            Fluent:SetTheme(themeTable)
        end)
    end
end

forceApplyDraconicTheme(Color3.fromRGB(255, 0, 0))
local Window = Fluent:CreateWindow({
    Title = "🌟 | Draconic-X-Remake",
    SubTitle = "Overhaul (2.9 Version) Made by Unknownproooolucky",
    TabWidth = 160,
    Size = UDim2.fromOffset(580, 460),
    Theme = DRACONIC_THEME_NAME,
...
SettingsTab:AddButton({
    ...
    Callback = function()
        SaveManager:Load()
        task.defer(function()
            forceApplyDraconicTheme(Color3.fromRGB(draconicAccentR, draconicAccentG, draconicAccentB))
        end)
        Fluent:Notify({
            Title = "Settings",
...
SettingsTab:AddSection("Draconic Theme")

local draconicAccentR = 255
local draconicAccentG = 0
local draconicAccentB = 0
local draconicThemeAnimated = false

local function applyDraconicThemeAccent()
    forceApplyDraconicTheme(Color3.fromRGB(draconicAccentR, draconicAccentG, draconicAccentB))
end

SettingsTab:AddSlider("ThemeAccentR", {
    Title = "Theme Red",
    Description = "Red channel for your theme accent",
    Default = draconicAccentR,
    Min = 0,
    Max = 255,
    Rounding = 0,
    Callback = function(value)
        draconicAccentR = math.floor(value)
        if not draconicThemeAnimated then
            applyDraconicThemeAccent()
        end
    end
})

SettingsTab:AddSlider("ThemeAccentG", {
    Title = "Theme Green",
    Description = "Green channel for your theme accent",
    Default = draconicAccentG,
    Min = 0,
    Max = 255,
    Rounding = 0,
    Callback = function(value)
        draconicAccentG = math.floor(value)
        if not draconicThemeAnimated then
            applyDraconicThemeAccent()
        end
    end
...
SettingsTab:AddSlider("ThemeAccentB", {
    Title = "Theme Blue",
    Description = "Blue channel for your theme accent",
    Default = draconicAccentB,
    Min = 0,
    Max = 255,
    Rounding = 0,
    Callback = function(value)
        draconicAccentB = math.floor(value)
        if not draconicThemeAnimated then
            applyDraconicThemeAccent()
        end
    end
...
SettingsTab:AddToggle("DraconicThemeAnimation", {
    Title = "Animated Theme",
    Description = "Use external theme animation like your buttons",
    Default = false,
    Callback = function(state)
        draconicThemeAnimated = state
        if state then
            loadedTheme.StartAnimation(RunService, function(newTheme)
                Fluent.Themes[DRACONIC_THEME_NAME] = newTheme
                Fluent:SetTheme(DRACONIC_THEME_NAME)
            end)
        else
            loadedTheme.StopAnimation()
            applyDraconicThemeAccent()
        end
    end
})
